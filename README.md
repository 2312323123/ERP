...
#### initializing roles created by the microservice
...
- if the new microservice sets up some new roles:
  ...
  - `nest g s`
  - `init_roles`
  - then copy init roles contents from some other service, i.e. auth_and_permissions one (and backup_service has some AI-refactored version)
  - for `HTTPService` to workyou need put `HttpModule` from @nestjs/axios among app.module.ts imports array
  - if it still doesn't see nest/axios in here `import { HttpService } from '@nestjs/axios';`, then for me restarting IDE after trying everything else has helped
...
(if `nest` command doesn't work, read the very beginning of the nestjs docs)
